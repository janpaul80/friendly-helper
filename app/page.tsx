...
export default function LandingPage() {
  ...
  const handleSend = () => {
    ...
    // Optimization: Navigate to workspace with query params
    router.push(`/dashboard`); // In this simplified clone, we go to dashboard first.
  };
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white font-sans selection:bg-orange-500/30 overflow-x-hidden">
      ...
      {/* Connectors Modal Placeholder */}
      {showConnectorsModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[100] p-4">
          <div className="bg-[#1a1a1a] border border-white/10 rounded-2xl max-w-2xl w-full max-h-[80vh] overflow-hidden relative shadow-2xl">
            <div className="p-6 border-b border-white/10 flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-bold text-white">Connectors</h2>
                <p className="text-gray-400 text-sm mt-1">Unlock more with HEFTCoder — connect your tools</p>
              </div>
            ...
            <div className="p-12 text-center">
              <p className="text-gray-500 font-mono">CONNECTOR_SYSTEM_OFFLINE: Requires Enterprise plan to activate local hooks.</p>
            </div>
          </div>
        </div>
      )}
                ...
                <div className="relative" ref={attachDropdownRef}>
                  ...
                  {showAttachDropdown && (
                    <div className="absolute bottom-full mb-2 left-0 bg-[#1a1a1a] border border-white/10 rounded-xl py-2 w-52 shadow-xl z-50">
                      <Link href="/login" className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-300 hover:bg-white/5 hover:text-white">
